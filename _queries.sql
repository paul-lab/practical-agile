...
('101','101','My Tasks','select concat( initials,\" - \" ,Friendly_Name) as User, story.ID as \'Story #\' ,task.Desc  as Task,IF(Done=0,\'-\',IF(Done=1,\'In Progress\',\'Done\')) as Status,  Expected_Hours as \'Est. hrs\', Actual_Hours as \'Act. hrs\' from Story left join iteration on story.Iteration_ID = Iteration.ID, Task  left JOIN user on task.User_ID = User.ID  where Story.Iteration_ID = {Iteration} and User_ID = {User} and task.Story_AID = story.AID order by story.Iteration_Rank, story.ID, Done','','2'),
('102','102','Iteration Tasks','select concat( initials,\" - \" ,Friendly_Name) as User, story.ID as \'Story #\' ,task.Desc  as Task,IF(Done=0,\'-\',IF(Done=1,\'In Progress\',\'Done\')) as Status,  Expected_Hours as \'Est. hrs\', Actual_Hours as \'Act. hrs\' from Story left join iteration on story.Iteration_ID = Iteration.ID, Task  left JOIN user on task.User_ID = User.ID  where Story.Iteration_ID = {Iteration} and task.Story_AID = story.AID order by story.Iteration_Rank, story.ID, User_ID,  Done','','2'),
('103','50','Project Progress','SELECT story.Type, story.Status, count(story.Type) as \'# Stories\', sum(story.Size) as \'# Points\' FROM story where story.Project_ID={Project} and ( 0 = (select count(*) from story as child where child.Parent_Story_ID=story.AID)) group by story.Type, story.Status','','2'),
('200','500','Audit (recent 200)','SELECT * from audit order by ID desc limit 200','','2'),
('210','510','Project Audit','SELECT * from audit where PID={Project} order by ID desc','','2'),
('220','520','Non Project Audit','SELECT * from audit where PID=0 order by ID desc','','2'),
('221','20','Release plan','(story.Release_ID > 0) and 0 = (select count(*) from story as child where child.Parent_Story_ID=story.AID) ','order by  Release_ID','1'),
